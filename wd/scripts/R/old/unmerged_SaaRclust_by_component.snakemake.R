...
parallel.results <- foreach (bam=bam.files, .packages=c('Rsamtools', 'data.table')) %dopar%{
  ...
  aln = scanBam(file = bam,
                param = ScanBamParam(
                  what = c('qname', 'rname', 'strand', 'pos', 'qwidth', 'mrnm', 'isize'),
                  flag = scanBamFlag(
...
counts.l.all <- lapply(parallel.results, function(l) l$counts)
...
wc.cells.clusters_all <-
  lapply(1:length(clust.pairs_by_component), function(i){
    get.wc.cells.clusters(soft.clust_by_component[[i]], clust.pairs_by_component[[i]], min.theta.wc=0.95)
  }) %>%
